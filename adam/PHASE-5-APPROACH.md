...
**Precondition (D-09):** No ADaM spec is finalised until the Statistical Analysis Plan (`sap/SAP.md`) and the TFL shells list (`tfl/TFL-SHELLS.md`) are locked at Gate 3.5. Every ADaM variable must trace to either an analysis in the SAP or a variable used by a TFL shell — this is what prevents ADaM specs from drifting away from the analyses they are supposed to support.
...
**Decision:** The Statistical Analysis Plan (`sap/SAP.md`) and the TFL shells list (`tfl/TFL-SHELLS.md`) must be written and LG-approved at Gate 3.5 before any ADaM spec is finalised.
...
**Precondition:** `sap/SAP.md` + `tfl/TFL-SHELLS.md` locked at Gate 3.5. Do not start any row below until Gate 3.5 passes.
