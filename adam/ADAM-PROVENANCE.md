...
| 0.9 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **Re-derived after the SDTM P21 structural cleanup; P21 ADaM 10,931 → 10,890, all residuals accepted (no AD-rule findings).** Cascade only: SDTM `04/16` AE/CM date clamps flow into `ADAE`/`ADCM` (dates only; ADAE 2,840, TRTEMFL/counts unchanged). Evaluated and **rejected** modelling never-dosed subject 0277 as ASSIGNED-NOT-TREATED — nulling `DM.ACTARM` cascaded into `ADSL.TRT01A=null` with `TRT01AN` populated (**AD1011 ×60**); reverted, so 0277 stays randomised-untreated (ITT yes, SAFFL=N) and ADSL is unchanged from 0.8. ADaM run now validates against the combined `define/define.xml` (both standards present → no SD0061). |
| 0.10 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **Baseline analysis visit + baseline selection specified.** Baseline selection is now stated in SAP §12.3 and the ADLB/ADVS/ADTR specs: the **last non-missing value on or before first dose** (`ADT ≤ TRTSDT`), per `USUBJID × PARAMCD` (ADTR also per lesion), ties by SDTM sequence → `ABLFL='Y'` — **date-based, not visit-restricted** (the baseline may be a Screening or a C1D1 pre-dose draw). Per CDISC ADaM convention (no CT governs `AVISIT`; the convention comes from the ADaMIG BDS examples and the ADaM Pilot) that record now carries **`AVISIT="Baseline"`, `AVISITN=0`**. The `AVISITN 0` window is labelled `Baseline` in *both* streams (was `SCREENING` for TREATMENT and `BASELINE` for TUMOUR — one visit number carrying two strings), and `apply_baseline_visit()` relabels the `ABLFL` record explicitly because day-windowing alone cannot guarantee it: a C1D1 pre-dose draw has `ADY ≈ 0` and windows to `C1D1`. Unwindowable records (no `ADY`, e.g. the untreated subject) now map their SDTM `VISITNUM` to the analysis label, so `AVISITN=0` only ever means `Baseline`. **Fixed a stale ADVS spec** whose D1 rule claimed a visit-restricted baseline (`AVISIT %in% c("SCREENING","SCR","C1D1")`) that never matched the implemented date-based code. Verified: ADLB 4,908 / ADVS 3,143 / ADTR 449 `ABLFL='Y'` records — all `AVISIT="Baseline"`, `AVISITN=0`, zero violations. |
| 0.11 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **`ANL01FL` unified across all 11 ADaM datasets + ALP parameter added.** `ANL01FL` now carries **one definition** — *the records selected for that dataset's primary analysis* — replacing a per-visit rule in some datasets and a bare `"Y"` in others. Windowed BDS (ADLB/ADVS/ADRS/ADTR) share `flag_anl01()`: one record per `USUBJID × PARAMCD [× extra_key] × AVISIT`, the `ABLFL` record inside the Baseline visit else closest to target (ties → later `ADT`); subject-level rows (ADRS `BOR`/`CBOR`) get one per `USUBJID × PARAMCD`; `extra_key` adds a finer level for ADTR, which is one row per **lesion** per visit and passes `LNKID`. This replaced ADTR's bespoke post-baseline-SDIAM rule and ADRS's blanket `"Y"` — SAP §12.2 had *already claimed* `flag_anl01()` was applied by all four, so the code now matches the spec. Non-by-visit datasets state their analysis population explicitly instead of a constant: ADAE `TRTEMFL & SAFFL`; ADCM/ADEX `SAFFL`; ADMH/ADDS/ADDV/ADTTE `ITTFL`. Only real data change: ADCM 2,319 → 2,312 flagged (the 7 are the never-dosed subject's con-meds). Also fixed **ADTR never assigning `ATPTREF`** from the windowing (ADLB/ADVS/ADRS did), which `flag_anl01` needs for distance-to-target ordering. **ALP** added as `PARAMCD="ALP"` (11,491 records) with CTCAE v5 grading (>ULN-2.5x / >2.5-5x / >5-20x / >20x → grade 0=10,250, 1=853, 2=388); its baseline records follow the new convention automatically (ADLB `ABLFL` 4,908 → 5,357, +449). Verified zero duplicate groups in all four BDS datasets. P21 unchanged: ADaM 10,890. |
