# SIMULATED-TORIVUMAB-2026 — Torivumab Phase 3 NSCLC Study

> ⚠️ **FICTIONAL EDUCATIONAL DOCUMENT — NOT FOR REGULATORY USE.**  
> All identifiers, patient data, drug names, and results are completely synthetic.  
> Celindra Therapeutics and torivumab are fictional. Do NOT submit to any regulatory authority or clinical registry.

A fully synthetic, end-to-end oncology Phase 3 clinical trial dataset conforming to CDISC standards.  
Developed as a contribution to the [`clinTrialData`](https://github.com/Lovemore-Gakava/clinTrialData) R package.

---

## Study Overview

| | |
|---|---|
| **Study ID** | SIMULATED-TORIVUMAB-2026 |
| **Short Title** | TORIVA-LUNG 301 |
| **Sponsor** | Celindra Therapeutics *(fictional)* |
| **Drug** | Torivumab 200 mg IV Q3W (anti-PD-L1 monoclonal antibody) *(fictional)* |
| **Indication** | Advanced/metastatic NSCLC, Stage IIIB/IV |
| **Population** | First-line, PD-L1 TPS ≥50%, no EGFR/ALK mutations |
| **Design** | Randomised, double-blind, placebo-controlled, multinational |
| **N** | 450 subjects — 225 torivumab : 225 placebo |
| **Randomisation** | 1:1 (torivumab : placebo), stratified by histology and region |
| **Primary Endpoint** | Overall Survival (OS) |
| **Secondary Endpoints** | PFS, ORR, Safety |
| **Response Criteria** | RECIST 1.1 |
| **Treatment Duration** | Up to 35 cycles (~2 years) or until PD/unacceptable toxicity |

---

## Pipeline Progress

| Phase | Description | Status |
|-------|-------------|--------|
| 1 | Protocol Synopsis | ✅ Complete (v1.1, 2026-03-30) |
| 2 | Annotated CRF (aCRF) | ✅ Complete — Gate 2 APPROVED (2026-04-01) |
| 3 | Simulated raw eCRF data | ✅ Complete — 15 domain CSVs (`programs/raw/`); v0.3 covariate-driven simulation incl. protocol deviations + unscheduled safety rechecks |
| 4 | SDTM (26 domains) | ✅ Complete — 26 Parquet domains: 22 tabulation (incl. DV Protocol Deviations) + 4 trial-design (TS/TA/TE/SE, added during P21 remediation), SDTMIG v3.4 labelled (`datasets/sdtm/`) |
| 4.5 | SAP v0.2 + TFL shells v0.3 | ✅ Complete — Gate 3.5 PASSED; SAP includes full ICH E9(R1) estimand framework |
| 5 | ADaM (12 datasets) | ✅ Complete — 6 efficacy/safety + 6 pharma-standard descriptive (ADDS, ADDV, ADEX, ADCM, ADVS, ADMH); date-based analysis-visit windowing + unified `ANL01FL` + baseline `AVISIT`/`AVISITN=0` convention (SAP §12.2/§12.3); alkaline phosphatase added as a full analyte |
| 6 | TFLs (43 outputs) | ✅ Complete — all 43 outputs in RTF + DOCX + HTML + PNG (`tfl/`) |
| **QC** | **Validation strategy + tooling + trackers** | ✅ **In place** — see `qc/VALIDATION-PLAN.md`, [GitHub issues #1–#9](https://github.com/OpenTrialReporting/torivumab-nsclc-301/issues); Pinnacle 21 CLI-validated (SDTM 10,891 / ADaM 10,890 findings — 10,873 the accepted SD0007 DA-units warning, every remaining finding a documented accepted limitation; see `qc/p21-reports/`) |
| 7 | CSR | ⏳ Phase 7 |
| 8 | ADRG | ⏳ Phase 8 |

**Bonus deliverables:** Define-XML v2.1 (`define/` — 38 datasets / 708 variables), submission-style TFL shell mockup (`sap/shells/TFL-SHELLS-DOC.docx`), double-programming mapping specs for SDTM + ADaM, plus 3 Excel QC programming trackers + 5 validation R scripts.

See [ROADMAP.md](ROADMAP.md) for full details and timelines.

---

## Repository Structure

```
torivumab-nsclc-301/
│
├── protocol/
│   └── synopsis.md (v1.1 — locked) ✅
│
├── crf/                                         ✅ Phase 2 — Gate 2 APPROVED
│   ├── SIMULATED-TORIVUMAB-2026_CRF.xlsx
│   ├── CRF_Annotated.html / .pdf / .Rmd
│   └── field_definitions.csv / visit_schedule.csv / codelist_reference.csv
│
├── raw/                                         ✅ Simulated eCRF data (generated by programs/raw/)
│   ├── demographics.csv … physical_exam.csv     15 domain CSVs (run 00_simulate_raw.R to create)
│   └── codelists/
│       ├── meddra_oncology_subset.csv           ~80 MedDRA terms (public-source)
│       ├── atc_conmed.csv                       ~40 medications (WHO ATC)
│       └── cdisc_ct.csv                        Key CDISC CT items
│
├── programs/                                    ✅ All derivation code — clean code/data separation
│   ├── raw/                                     Phase 3: simulate eCRF data
│   │   ├── 00_simulate_raw.R                   Master controller (set.seed = 20260301)
│   │   ├── 01_demographics.R … 13_physical_exam.R
│   │   └── RAW-PROVENANCE.md
│   ├── sdtm/                                   Phase 4: map raw → SDTM
│   │   ├── 00_run_sdtm.R                       Master controller
│   │   ├── dm.R, ds.R, ex.R, ae.R, cm.R,
│   │   │   lb.R, vs.R, mh.R, pe.R, tu.R,
│   │   │   tr.R, rs.R, dd.R, su.R, da.R,
│   │   │   relrec.R, supp*.R, + trial design
│   │   │   (ts.R, ta.R, te.R, se.R)
│   │   └── SDTM-PROVENANCE.md
│   ├── adam/                                   ✅ Phase 5 COMPLETE: all 12 ADaM scripts
│   │   ├── 00_run_adam.R                       Master controller
│   │   ├── adsl.R, adae.R, adlb.R, adtr.R, adrs.R, adtte.R   (6 efficacy/safety)
│   │   ├── adcm.R, adds.R, addv.R, adex.R, admh.R, advs.R   (6 pharma-standard descriptive)
│   │   └── PHASE-5-APPROACH.md
│   └── tfl/                                    ✅ Phase 6 COMPLETE: 43 outputs
│
├── datasets/                                   Outputs only — no code
│   ├── sdtm/  *.parquet (26 domains)           ✅ Generated by programs/sdtm/ (22 tabulation + 4 trial-design)
│   └── adam/  *.parquet (12 datasets)          ✅ Generated by programs/adam/
│
├── programming-specs/                          Per-dataset specs for QC double programming
│   ├── README.md                               Inventory + standards + template
│   ├── SDTM-DM-spec.md … SDTM-DV-spec.md       22 SDTM derivation specs (trial-design domains carry no derivation)
│   └── ADSL-spec.md … ADVS-spec.md             12 ADaM dataset specs
│
├── sap/                                        ✅ Phase 4.5 — SAP locked
│   ├── SAP.md
│   ├── SAP-PROVENANCE.md
│   └── shells/                                 TFL shells (SAP artefact, not production code)
│       ├── shells.yaml / TFL-SHELLS-DOC.docx
│       ├── render_shells_doc.R / validate_shells.R
│       └── SHELLS-PROVENANCE.md
│
├── tfl/                                        ✅ Phase 6 outputs (43 TFLs, all formats)
│   ├── tables/    *.rtf, *.docx, *.html        (32 tables × 3 formats incl. T-DV-01 deviations)
│   ├── figures/   *.png                         (6 figures: 2× KM, waterfall, spider, forest, swimmer)
│   ├── listings/  *.html|.docx|.rtf            (5 listings: SAE, deaths, AE-DISC, G3+ labs, deviations)
│   ├── TFL-OUTPUTS.html                         (combined browser view)
│   └── TFL-OUTPUTS.docx                         (combined Word view)
│
├── adam/                                       ADaM provenance and session info
│   ├── ADAM-PROVENANCE.md
│   └── session_info_install.txt
│
├── define/                                     ✅ Define-XML v2.1 (define.xml + summary, 38 datasets)
├── qc/                                         ✅ Validation strategy + tracking + tooling
│   ├── VALIDATION-PLAN.md                       (SOP — read first)
│   ├── README.md                                (workflow + tooling overview)
│   ├── SDTM-PROGRAMMING-TRACKER.xlsx            (22 rows)
│   ├── ADAM-PROGRAMMING-TRACKER.xlsx            (12 rows)
│   ├── TFL-PROGRAMMING-TRACKER.xlsx             (43 rows, with estimand IDs)
│   └── reports/                                 (auto-generated; gitignored)
├── programs/qc/                                R scripts for validation execution
│   ├── build_trackers.R, _compare_helpers.R
│   ├── run_reproducibility_check.R
│   ├── compare_sdtm.R, compare_adam.R
│   └── extract_tfl_values.R
├── csr/                                        Phase 7 (pending)
├── NOTES.md / ROADMAP.md / AGENTS.md
└── README.md
```

---

## Standards

| Standard | Version | Purpose |
|----------|---------|---------|
| CDASH | v2.1 | CRF data collection |
| SDTMIG | v3.4 | Study data tabulation |
| ADaMIG | v1.3 | Analysis data model |
| CDISC CT | 2024-03 | Controlled terminology |
| RECIST | 1.1 | Oncology response criteria |
| Define-XML | v2.1 | Dataset metadata |
| MedDRA | v27.0 | Adverse event coding |
| CTCAE | v5.0 | Toxicity grading |

---

## Pharmaverse Stack

`admiral` · `admiralonco` · `metacore` · `metatools` · `xportr` · `rtables` · `tern` · `teal`

---

## Reproducible Environment (uvr)

This project uses **[uvr](https://github.com/uvr-org/uvr)** for fast, reproducible R package management — a single lockfile pins every dependency at exact versions for regulatory auditability.

### Quick Start

```bash
# Install uvr (one-time)
curl -LsSf https://astral.sh/uv/install.sh | sh

# Clone and sync
git clone https://github.com/OpenTrialReporting/torivumab-nsclc-301.git
cd torivumab-nsclc-301
uvr sync
```

`uvr sync` reads `uvr.lock` and installs all 160 packages (34 direct, 126 transitive) into `.uvr/library/`. The `.Rprofile` auto-links the library on R startup — `library(admiral)` just works.

### Key Commands

| Command | Purpose |
|---|---|
| `uvr sync` | Install exact versions from lockfile |
| `uvr add <pkg>` | Add a new dependency + update lockfile |
| `uvr scan` | Detect packages used in R/Rmd scripts not yet declared |
| `uvr tree` | Show full dependency tree |
| `uvr lock --check` | Verify lockfile is up to date (CI) |

### Lockfile

`uvr.lock` (1,975 lines) is the single source of truth for package versions. It is committed to git and should be reviewed in PRs. The `.uvr/library/` directory is gitignored — each developer runs `uvr sync` to populate it locally.

---

## License

Synthetic data — educational use only. CC BY 4.0.
