...
| 0.4 | 2026-09-20 | LG (w/ Claude Fable 5.1) | **Reference documents re-pointed after Amendment 1 (#27).** `reference_documents` now `PROTO-1.2` (Protocol v1.2, 2026-09-20) and `SAP-0.3` (SAP v0.3); every output's reference list updated by id; `shells_version` 0.4. No shell content changed. Closes the shells half of #53. |
...
- ~~**ADTTE `PARAMCD='OSWOT'` not yet derived**~~ — ✅ Resolved same day. `programs/adam/adtte.R` now derives `PARAMCD='OSWOT'` for all ITT subjects. Event = death on or within 30 days of last study treatment (TRTEDT + 30 days). Censor = `min(TRTEDT + 30d, LSTALVDT)`. Validated via Cox PH: HR 0.476 (95% CI 0.374-0.607), 277 events out of 450, median TRT 586d vs PBO 412d — stronger effect than the full-OS estimand (HR 0.567) as expected when the treatment-period effect is isolated from late off-treatment dilution. T-EFF-12 is now fully programmable.
- **Subsequent-anti-cancer-therapy capture** — T-DS-03 and the SAP §13.4 OSWOT specification both reference subsequent anti-cancer therapy as a censoring trigger, but the synthetic CM data (raw/codelists/atc_conmed.csv) covers supportive care only and does not simulate post-trial anti-cancer therapy. The current OSWOT derivation therefore applies only the `TRTEDT + 30d` half of the SAP rule. For real-world data, extend `adtte.R` with a subsequent-therapy CM lookup and take `pmin` with the existing OSWOT_CENSOR. Documented in `programming-specs/ADTTE-spec.md` PARAMCD table.
