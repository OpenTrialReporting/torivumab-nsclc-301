...
- **TREATMENT** (ADLB, ADVS) — Q3W schedule: `SCREENING` (target day −14),
...
`VISITNUM` scheme (Screening 0; induction `C1D1…C6D1` 1…7; maintenance
`MAINT_CnD1` = 9 + n; tumour baseline 0; `TUMOR_ASSESS_WKn` = n; EOT / FU1 / FU2
= 900 / 901 / 902).
...
filter `ANL01FL = 'Y'`. Baseline (`ABLFL`) is derived independently from the date
(last non-missing value on/before first dose) and is unaffected by windowing.
...
`C1D1`/baseline visit as their nearest scheduled timepoint.
