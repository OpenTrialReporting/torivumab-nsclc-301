...
- Time to event: `AVAL = DTHDT - RANDDT + 1` (days; `AVALU = "DAYS"`, the ADaM convention for ADTTE). `AVAL` is converted to months as `AVAL / 30.4375` for reporting — months are a reporting-layer transformation and are not stored in ADTTE. OS, PFS and PFSINV anchor to `RANDDT`; OSWOT (estimand E1b, while-on-treatment) anchors to `TRTSDT`, which is what distinguishes it from E1.
