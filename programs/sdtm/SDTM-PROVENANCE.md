...
| 0.6 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **P21 structural cleanup (SDTM 10,981 → 10,891; all residuals now accepted/documented).** Variable order aligned to the SDTMIG 3.4 sequence via a new `SDTM_ORDER` table + `apply_order()` relocate in `16_label_domains.R` (**SD1079** 32→0). **AE/CM date integrity** (**SD0080/SD1202/SD1204** 37→0): raw `04_adverse_events.R` clamps AE start/end to the subject's disposition-event (exit) date and `16_subsequent_therapy.R` caps subsequent-therapy start at last contact — clamps applied to sampled *results*, not `sample()` arguments, so the RNG stream (and the rest of the simulated data) is unchanged. Added Expected variables `EX.EXDOSFRM`, `LB.LBORNRLO/HI` (character), `LB.LBLOBXFL`, `PE.PESTRESC`, `TR.TRORRESU/TRMETHOD/TREVAL/TRLOBXFL` (**SD0057** 17→8). **Define split per standard**: `build_define.R` now also emits `define/sdtm/define.xml` (SDTM-only), which the SDTM run uses so ADaM datasets are no longer reported missing (**SD0061** 12→0). Rejected: modelling never-dosed subject 0277 as ASSIGNED-NOT-TREATED (caused more findings than it cleared) and populating `TS.TSVALCD/TSVCDREF` (triggered SD2240-series). |
| 0.7 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **Alkaline phosphatase (ALP) mapped into LB.** `lb.R` gains the `ALP → "Alkaline Phosphatase"` entry in the `LBTESTCD → LBTEST` recode, so the new raw analyte (RAW-PROVENANCE 0.7) carries the exact CDISC decode required by P21 CT2003; `LBCAT` resolves to `CHEMISTRY` via the existing haematology-code test. **LB 124,751 → 136,242 records** (+11,491), SUPPLB 249,502 → 272,484. Because ALP sorts before ALT, `LBSEQ` renumbers across the domain and the SUPPLB/RELREC `IDVARVAL` references follow — regenerated together. P21 unchanged: SDTM 10,891 (no new CT, order or conformance findings from the added analyte). |
