...
| 0.5 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **Unscheduled visits added to labs + vitals (collection-realistic).** `07_labs.R`: an abnormal on-treatment safety lab (ALT/AST/NEUT/PLAT/HGB/BILI/CREAT H/L) prompts an off-schedule recheck ~1 week later. `08_vital_signs.R`: an abnormal on-treatment vital (SBP≥160 / DBP≥100 / HR≥110) prompts a recheck. Both emit `VISIT_NAME="UNSCHEDULED"` with the real off-schedule date and recovering values, generated in a dedicated RNG stream (seeds 20260707 / 20260708, with `.Random.seed` save/restore) and constrained to be genuinely off-schedule and within participation — so the scheduled data and every downstream raw program stay byte-for-byte identical. ~2,150 unscheduled lab rows + 8 unscheduled vitals visits. Flows through SDTM (`VISIT="UNSCHEDULED"`, `VISITNUM=998`) → ADaM windowing (SAP §12.2). |
| 0.7 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **Alkaline phosphatase (ALP) added to the chemistry panel.** `07_labs.R` emits one ALP result (U/L, reference 40-130, ~8% cholestatic/bone-metastatic elevation at 1.2-3.5x ULN) at every scheduled collection already in the data — 11,491 rows, 1,241 flagged H. Closes a spec-vs-data gap: `ADLB-spec.md` already listed alkaline phosphatase in the CTCAE panel but the analyte was never simulated. Generated in a **dedicated RNG stream** (seed 20260709, `.Random.seed` save/restore) and appended, so every previously simulated lab value and every downstream raw program stayed byte-for-byte identical — `labs.csv` was the only raw file to change. Flows to SDTM `LBTESTCD="ALP"` / `LBTEST="Alkaline Phosphatase"` (exact CDISC decode, LBCAT=CHEMISTRY) and ADaM `PARAMCD="ALP"` with CTCAE v5 grading (>ULN-2.5x / >2.5-5x / >5-20x / >20x). |
| 0.6 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **AE/CM dates constrained to the participation window (P21 SD0080/SD1202/SD1204).** `02_disposition.R` exposes a per-subject `disp_event_date` (death / discontinuation / completion — the date that becomes the latest `DS.DSSTDTC`). `04_adverse_events.R` clamps every generated AE start/end to that exit date (fixes the early-death fallback that could place an AE after death). `16_subsequent_therapy.R` caps subsequent-therapy start at last contact, dropping records whose latent progression falls beyond follow-up. **RNG-neutral**: clamps are applied to the sampled *results*, never to `sample()` arguments (R's post-3.6 rejection sampler consumes a variable number of draws per call, so changing a range shifts the whole stream) — regeneration touched only `adverse_events.csv` (67 rows) and `conmed.csv` (11 rows dropped); every other raw CSV stayed byte-identical. |
