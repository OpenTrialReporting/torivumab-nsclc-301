...
| 0.7 | 2026-07-19 | LG (w/ Claude Opus 4.8 1M) | **Regenerated after the Phase-3 SDTM/ADaM changes; all 43 outputs still build.** No TFL program edits were required — the changes flow through the existing derivations: the new **alkaline phosphatase** analyte lifts **T-LB-01 from 11 → 12 parameters** and **T-LB-02 from 6 → 7**; the AE/CM date clamps and the `AVISIT="Baseline"` relabel shift AE/con-med listings and by-visit lab/vitals tables; the unified `ANL01FL` leaves `t_vs_01_summary` (the only TFL that filters it) unchanged in meaning, since ADVS already used the shared rule. Efficacy outputs are unmoved (OS HR 0.575, PFS HR 0.506, ITT ORR 28.2%), confirming the structural work did not disturb the analysis results. |
